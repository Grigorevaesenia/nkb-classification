...
class InferDataset(Dataset):
    def __init__(
        ...
        self.train_ann_table = self.train_ann_table[
            self.train_ann_table["fold"] == "train"
        ]
        self.target_names = [*sorted(target_names)]
        self.classes = {
            target_name: np.sort(
                np.unique(self.train_ann_table[target_name].values)
            ).tolist()
            for target_name in self.target_names
        }
        self.class_to_idx = {
            target_name: {k: i for i, k in enumerate(classes)}
...
class GroupsDataset(Dataset):
    ...
    def load_annotations(self):
        ...
        self.class_to_idx = {
            k: i for i, k in enumerate(group_dict.keys())
        }
        self.idx_to_class = {
            idx: lb for lb, idx in self.class_to_idx.items()
        }
        ...
        for sample in data:
            ...
            img_path = Path(
                self.data_prefix, "images_lr", orig_label, filename
            )
            assert (
                img_path.is_file()
            ...
            info = {
                ...
                "gt_label": np.array(
                    self.class_to_idx[label], dtype=np.int64
                ),
            }
...
class AnnotatedMultitaskDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.classes = {
            target_name: np.sort(
                np.unique(self.table[target_name].values)
            ).tolist()
            for target_name in self.target_names
        }
        self.class_to_idx = {
    ...
    def __getitem__(self, idx):
        ...
        labels = {
            target_name: np.array(
                self.class_to_idx[target_name][
                    self.table.iloc[
                        idx, self.table.columns.get_loc(target_name)
                    ]
                ],
                dtype=np.int64,
            )
